---
{"dg-publish":true,"dg-path":"数学/复变函数/泰勒级数.md","permalink":"/数学/复变函数/泰勒级数/","dgPassFrontmatter":true,"noteIcon":"","created":"2024-05-21T15:20:28.442+08:00","updated":"2024-06-12T19:55:07.337+08:00"}
---
...
$$\sum\limits_{n=0}^{\infty}\dfrac{f^{(n)}(x_{0})}{n!}(x-x_{0})^{n}=f(x_{0})+f'(x-x_{0})+\dfrac{f''(x-x_{0})}{2!}+\cdots+\dfrac{f^{(n)}(x_{0})}{n!}(x-x_{0})+\dots$$
称为在点 $x_{0}$ 处的泰勒级数
